...
@Path("/components")
public class BrandExamplesRoutes {
    ...
    @Location("components/brand/_standalone")
    @Inject
    Template standalonePage;

    @GET
    @Path("/brand/source/{example}")
    @Produces(MediaType.TEXT_PLAIN)
    public String source(@PathParam("example") String example) {
        if (!EXAMPLES.contains(example)) {
            throw new NotFoundException("Unknown brand example: " + example);
        }
        String resourcePath = "/templates/components/brand/" + example + ".html";
    ...
    @GET
    @Path("/brand/source-java/{example}")
    @Produces(MediaType.TEXT_PLAIN)
    public String sourceJava(@PathParam("example") String example) {
        if (!EXAMPLES.contains(example)) {
            throw new NotFoundException("Unknown brand example: " + example);
        }
        String resourcePath = "/code-samples/brand/" + example + ".java";
        try (InputStream in = getClass().getResourceAsStream(resourcePath)) {
            if (in == null) {
    ...
    public TemplateInstance standalone(@PathParam("example") String example) {
        ...
        Template inner = engine.getTemplate("components/brand/" + example);
        if (inner == null) {
