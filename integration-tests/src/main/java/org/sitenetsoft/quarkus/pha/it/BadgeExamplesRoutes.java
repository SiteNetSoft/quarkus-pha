...
@Path("/components")
public class BadgeExamplesRoutes {
    ...
    @Location("components/badge/_standalone")
    @Inject
    Template standalonePage;

    @GET
    @Path("/badge/source/{example}")
    @Produces(MediaType.TEXT_PLAIN)
    public String source(@PathParam("example") String example) {
        if (!EXAMPLES.contains(example)) {
            throw new NotFoundException("Unknown badge example: " + example);
        }
        String resourcePath = "/templates/components/badge/" + example + ".html";
    ...
    @GET
    @Path("/badge/source-java/{example}")
    @Produces(MediaType.TEXT_PLAIN)
    public String sourceJava(@PathParam("example") String example) {
        if (!EXAMPLES.contains(example)) {
            throw new NotFoundException("Unknown badge example: " + example);
        }
        String resourcePath = "/code-samples/badge/" + example + ".java";
        try (InputStream in = getClass().getResourceAsStream(resourcePath)) {
            if (in == null) {
    ...
    public TemplateInstance standalone(@PathParam("example") String example) {
        ...
        Template inner = engine.getTemplate("components/badge/" + example);
        if (inner == null) {
