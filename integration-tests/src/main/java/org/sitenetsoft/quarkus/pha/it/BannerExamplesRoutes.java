...
@Path("/components")
public class BannerExamplesRoutes {
    ...
    @Location("components/banner/_standalone")
    @Inject
    Template standalonePage;

    @GET
    @Path("/banner/source/{example}")
    @Produces(MediaType.TEXT_PLAIN)
    public String source(@PathParam("example") String example) {
        if (!EXAMPLES.contains(example)) {
            throw new NotFoundException("Unknown banner example: " + example);
        }
        String resourcePath = "/templates/components/banner/" + example + ".html";
    ...
    @GET
    @Path("/banner/source-java/{example}")
    @Produces(MediaType.TEXT_PLAIN)
    public String sourceJava(@PathParam("example") String example) {
        if (!EXAMPLES.contains(example)) {
            throw new NotFoundException("Unknown banner example: " + example);
        }
        String resourcePath = "/code-samples/banner/" + example + ".java";
        try (InputStream in = getClass().getResourceAsStream(resourcePath)) {
            if (in == null) {
    ...
    public TemplateInstance standalone(@PathParam("example") String example) {
        ...
        Template inner = engine.getTemplate("components/banner/" + example);
        if (inner == null) {
