...
test.describe("Data view extensions", () => {
  ...
  test.describe("Table", () => {
    ...
    test("clicking a header sorts that column ascending then descending", async ({ page }) => {
      ...
      await expect(root.locator("tbody tr").first().locator("td").first()).toContainText("Alpine.js");
      await nameHeader.click();
      // Descending: 'video.js' should be first.
      await expect(root.locator("tbody tr").first().locator("td").first()).toContainText("video.js");
    });
    ...
    test("pagination advances to next page", async ({ page }) => {
      const root = page.locator("#dv-table");
      const firstRowText = await root.locator("tbody tr").first().locator("td").first().textContent();
      await root.getByRole("button", { name: "Next page" }).click();
      const nextFirstRowText = await root.locator("tbody tr").first().locator("td").first().textContent();
